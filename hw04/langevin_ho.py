...
kB = 1.0            
T = 1.0               
sqrt_coeff = np.sqrt(gamma * kB * T * dt / 2)
...
for t in times:
    ...
    p_half = p + (-x - gamma * p) * dt / 2 + sqrt_coeff * R1
    x_new = x + p_half * dt
    p_new = p_half + (-x_new - gamma * p_half) * dt / 2 + sqrt_coeff * R2
...
ax[1,1].plot(momentum_linear, 1./np.sqrt(np.pi)*np.exp(-1./(kB*T) * momentum_linear**2), label='Boltzmann distribution', color='k')
ax[1,0].plot(momentum_linear, 1./np.sqrt(np.pi)*np.exp(-1./(kB*T) * momentum_linear**2), label='Boltzmann distribution', color='k')
ax[1,0].legend()
