...
@njit
def update_thermostats(momenta: np.array, kB: float, T: float, Q: float, dt: float, p: float) -> np.array:
    ...
    if length == 1:
        ...
        for i in range(length):
            if i == length - 1:
                ...
                momenta[i] += (momenta[i-1]**2/Q - kB*T - momenta[i+1]/Q*momenta[i])*dt
...
@njit
def run_sim(update_thermostats, dt, TIME, times, kB, Q, x, p, TURNON, MOMENTA_THERMOSTAT, ETAS, x_vals, p_vals):
...
(x_vals, p_vals) = run_sim(update_thermostats, dt, TIME, times, kB, Q, x, p, TURNON, MOMENTA_THERMOSTAT, ETAS, x_vals, p_vals)
...
ax[1, 1].plot(
    momentum_linear,
    1.0 / np.sqrt(np.pi) * np.exp(-1.0 / (kB * T) * momentum_linear**2),
